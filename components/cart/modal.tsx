...
export default function CartModal() {
  ...
  const [isTurboNativeUserAgentDetected, setIsTurboNativeUserAgentDetected] = useState<Boolean>(false);

  const cart = cartDetail?.cart;
  const { isOpen, onOpen, onOpenChange } = useDisclosure();

  useEffect(()=>{
    // Trigger cart quantity update to hotwire native device.
    sendCartCountToNativeApp(( Number(cart?.itemsQty) || 0 ));
  }, [cart?.itemsQty]);

  useEffect(()=>{
    if(isTurboNativeUserAgent()) {
      setIsTurboNativeUserAgentDetected(true);
    }
  }, [isTurboNativeUserAgent]);

  // Function to send the cart count to native apps
  ...
  return (
    <>
      ...
      <button
        ...
        className={ ! isTurboNativeUserAgentDetected ? clsx(isLoading ? "cursor-wait" : "cursor-pointer") : 'hidden' }
        disabled={isLoading}
        onClick={onOpen}
      >
        { ! isTurboNativeUserAgentDetected && <OpenCart quantity={cart?.itemsQty} /> }
      </button>
      <Drawer
        backdrop="blur"
